...
async function main(): Promise<void> {
    ...
    if (borrowTokens.length != 0) {
        if (borrowTokens.length > 1) {
            ...
            //Calldata from flashLaonToken to debtToken
    ...
    //EnsoCalldata from collateral token to flashLoanToken
    ...
    const tx = await withdrawManager.connect(owner4).populateTransaction.withdraw(
        ...
        {
            _factory: "0x306F06C147f064A010530292A1EB6737c3e378e4",
            _token0: thenaPoolInfo._token0, // Pool token 0
            _token1: thenaPoolInfo._token1, // Pool token 1
            _flashLoanToken: thenaPoolInfo._flashLoanToken, // One if the pool token(token0/token1)
            _bufferUnit: bufferUnit.toString(),
            _solverHandler: ensoHandler.address,
            _flashLoanAmount: flashLoanAmounts,
            firstSwapData: [encodedParameters], //used when repay using enso, swap flashloan token to debt token to repay
            secondSwapData: [encodedParameters1], //used when repay using enso, swap collateral token to flashlaon token to pay loan back
            _swapHandler: swapHandler.address,
            _poolFees: [[]], //(used when dexRepayment is true) Pool fee should be v3 pool we want to include for swapping, flashlaon token to  underlying collateral token to flashLoanToken(current scenario swpaping btc to usdt, to repay the laon)
