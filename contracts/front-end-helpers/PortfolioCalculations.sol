  ...
  function getFlashLoanFeeFromPool(
    ...
    function getCollateralAmountToSell(
        ...
        for (uint256 i; i < _protocolToken.length; i++) {
            uint256 borrowBalance = IVenusPool(_protocolToken[i])
  ...
  function calculateDebtAndPercentage(
    ...
    function getAaveCollateralAmountToSell(
        ...
        for (uint256 i; i < _protocolToken.length; i++) {
            ...
            uint borrowBalance = currentVariableDebt *
                10 **
                    (18 -
                        IERC20MetadataUpgradeable(_underlyingToken).decimals());
        ...
        )
    {
        ...
        uint256 totalPrice = (borrowBalance *
            params.afterFeeAmount *
            oraclePrice) / params.totalSupplyPortfolio;
    ...
    function getAllDebtAmount(
        address _comptroller,
        address _venusAssetHandler,
        address _user
    ) external view returns (uint256[] memory, address[] memory) {
        address[] memory borrowedTokens = IAssetHandler(_venusAssetHandler)
            .getBorrowedTokens(_user, _comptroller);
        uint256 tokenCount = borrowedTokens.length;
        uint256[] memory debtAmounts = new uint256[](tokenCount);

        for (uint i = 0; i < tokenCount; i++) {
            debtAmounts[i] = this.getDebtAmount(borrowedTokens[i], _user);
        }
        return (debtAmounts, borrowedTokens);
    }
    ...
    function getDebtAmount(
        ...
        uint256 debtAmount = IVenusPool(_debtToken).borrowBalanceStored(_user);
        return debtAmount;
    }
}
